...
try:
    ...
    elogo = xbmc.translatePath('special://home/addons/plugin.video.bedaub/resources/art/bigx.png')
    dialog = xbmcgui.Dialog()
    ok=dialog.ok('[B][COLOR=FF67cc33]Bedaub Import Error[/COLOR][/B]','Failed To Import Needed Modules',str(e),'Report missing Module by tweting [COLOR=FF67cc33]@bedaub[/COLOR]')
    xbmc.log('Bedaub ERROR - Importing Modules: '+str(e), xbmc.LOGERROR)
...
def MAIN():
    ...
    for index, value in sorted(enumerate(d), key=lambda x:x[1]):
        ...
        if index==0:
            ...
            main.addDirHome("Bedaub's How To Guides",'how',16,art+'/howto.png')
        elif index==25:
            ...
            main.addDirHome('Mash Sports','https://raw.github.com/bedaub/BedaubSports/master/BedaubSports.xml',182,art+'/mashsports.png')
        elif index==33:
            ...
            if selfAddon.getSetting("stracker") == '0':
                main.addDirHome("SideReel Show Tracker",'Bedaub',397,art+'/sidereel.png')
            elif selfAddon.getSetting("stracker") == '1':
                main.addDirHome("Trakt Show Tracker",'Bedaub',429,art+'/trakt.png')
            else:
                main.addDirHome("SideReel Show Tracker",'Bedaub',397,art+'/sidereel.png')
                main.addDirHome("Trakt Show Tracker",'Bedaub',429,art+'/trakt.png')
    ...
    main.addPlayc('Click Me!!!','https://raw.github.com/bedaubBedaubArtwork/master/skins/vector/donation.png',244,art+'/paypal.png','','','','','')
    main.addSpecial('@bedaub','','',art+'/twitter.png')
    main.addPlayc('Bedaub Settings','http://www.movie25.so/',1999,art+'/MashSettings.png','','','','','')
              
def Announcements():
    #Announcement Notifier from xml file
    try:
        import time
        link=main.OPENURL('https://raw.github.com/bedaub/BedaubNotifications/master/Notifier.xml?'+ str(time.time()),verbose=False)
        ...
        if match:
            ...
            if new != ' ':
                ...
                if not os.path.exists(notified):
                    ...
                    TextBoxes("[B][COLOR red]Bedaub Announcements[/B][/COLOR]",anounce)
                if old != ' ':
        ...
        if new != ' ':
            ...
            if not os.path.exists(notified):
                ...
                if '</mu>' in video:
                    ...
                    movieplaylist.MLink2('Bedaub',video,'',muvideo=True)
                else:
                    from resources.libs import youtube
                    youtube.YOULink('Bedaub',video,'')
            if old != ' ':
                notified=os.path.join(runonce,str(old))
                if  os.path.exists(notified):
                    os.remove(notified)
...
def CheckForAutoUpdate(force = False):
	if selfAddon.getSetting("autosource") == "false":
		...
		if verCheck == True:
			...
			try:
				print "Bedaub auto update - started"
			...
			if locver < gitver and (not os.path.exists(RunningFilePath) or os.stat(RunningFilePath).st_mtime + 120 < time.time()) or force:
				...
				xbmc.executebuiltin("XBMC.Notification(Bedaub Update,New Update detected,3000,"+main.slogo+")")
				xbmc.executebuiltin("XBMC.Notification(Bedaub Update,Updating...,3000,"+main.slogo+")")
				...
				if os.path.isfile(UpdateLocalFile):
					...
					if autoupdate.unzipAndMove(UpdateLocalFile,extractFolder,pluginsrc):
						...
						print "Bedaub auto update - update install successful ("+str(gitver)+")"
						xbmc.executebuiltin("XBMC.Notification(Bedaub Update,Successful,5000,"+main.slogo+")")
						...
						print "Bedaub auto update - update install failed ("+str(gitver)+")"
						xbmc.executebuiltin("XBMC.Notification(Bedaub Update,Failed,3000,"+main.elogo+")")
						main.GA("Autoupdate",str(gitver)+" Failed")
				else:
					print "Bedaub auto update - cannot find downloaded update ("+str(gitver)+")"
					xbmc.executebuiltin("XBMC.Notification(Bedaub Update,Failed,3000,"+main.elogo+")")
					main.GA("Autoupdate",str(gitver)+" Repo problem")
				try:os.remove(RunningFilePath)
				except:pass
			else:
				if force: xbmc.executebuiltin("XBMC.Notification(Bedaub Update,Bedaub is up-to-date,3000,"+main.slogo+")")
				print "Bedaub auto update - Bedaub is up-to-date ("+str(locver)+")"
			return
	else:
...
def Notify():
    ...
    if not os.path.exists(notified):
        ...
        TextBoxes("[B][COLOR red]Bedaub Changelog[/B][/COLOR]",chlg)
        mashup=mashup-1
        notified=os.path.join(runonce,str(mashup))
        if  os.path.exists(notified):
            os.remove(notified)
...
def GlobalFav():
    if selfAddon.getSetting("groupfavs") == "true":
        ...
        main.addLink("[COLOR red]Bedaub Fav's can also be favorited under XBMC favorites[/COLOR]",'','')
        main.addDir("Downloaded Content",'Bedaub',241,art+'/downloadlog.png')
...
def TV():
    ...
    if selfAddon.getSetting("stracker") == '0':
        main.addDir("SideReel Show Tracker",'Bedaub',397,art+'/sidereel.png')
    elif selfAddon.getSetting("stracker") == '1':
        main.addDir("Trakt Show Tracker",'Bedaub',429,art+'/trakt.png')
    else:
        main.addDir("SideReel Show Tracker",'Bedaub',397,art+'/sidereel.png')
        main.addDir("Trakt Show Tracker",'Bedaub',429,art+'/trakt.png')
...
def ThreeDsec():
    ...
    link=getListFile('https://raw.github.com/bedaub/BedaubNotifications/master/Directories/3D_Directory.xml', os.path.join(CachePath,'ThreeD'))
    link=link.replace('\r','').replace('\n','').replace('\t','').replace('&nbsp;','')
...
def SPORTS():
    ...
    link=getListFile('https://raw.github.com/bedaub/BedaubNotifications/master/Sport_Directory.xml', os.path.join(CachePath,'Sports'))
    link=link.replace('\r','').replace('\n','').replace('\t','').replace('&nbsp;','')
    match=re.compile('<name>(.+?)</name><link>(.+?)</link><thumbnail>(.+?)</thumbnail><mode>(.+?)</mode>').findall(link)
    for name,url,thumb,mode in match:
        if re.findall('http',thumb):
...
def HOWTOCAT():
    main.addDir("Bedaub Installation Instructions",'PLzXXwZxGnHxbv-UVWa-TIoFUIXWCaY_rm',205,art+'/howto.png')
    main.addDir("Bedaub Tutorials",'PLzXXwZxGnHxaup4QuaJOk9lC5Oms9L6NL',205,art+'/howto.png')
    main.addDir("Bedaub Features & Reviews",'PLzXXwZxGnHxZS61kC2t0RdT-4ZcX44Ltd',205,art+'/howto.png')
...
def LiveStreams():
    ...
    link=getListFile('https://raw.github.com/bedaub/BedaubNotifications/master/LiveDirectory.xml',os.path.join(CachePath,'LiveStreams'))
...
def PlaylistDir():
    link=getListFile('https://raw.github.com/bedaub/BedaubNotifications/master/MoviePlaylist_Dir.xml',os.path.join(CachePath,'Playlist'))
    link=link.replace('\r','').replace('\n','').replace('\t','').replace('&nbsp;','')
    match=re.compile('<name>(.+?)</name><link>(.+?)</link><thumbnail>(.+?)</thumbnail><mode>(.+?)</mode>').findall(link)
    for name,url,thumb,mode in match:
        if re.findall('http',thumb):
...
def MAINDEL(murl):
    if 'packages' in murl:
        dialog = xbmcgui.Dialog()
        ret = dialog.yesno('Bedaub', 'Are you sure you want to delete packages folder','','','No', 'Yes')
        if ret:
            import os 
    ...
    if 'Malformed' in murl:
        xbmc.executebuiltin('XBMC.RunScript(special://home/addons/plugin.video.bedaub/resources/fixes/malformedFix.py)')
        ...
        dialog = xbmcgui.Dialog()
        if dialog.yesno('Bedaub', 'Are you sure you want to clear XBMC cache ?','','','No', 'Yes'):
        ...
        dialog = xbmcgui.Dialog()
        if dialog.yesno('Bedaub', 'Are you sure you want to clear Bedaub\'s Cache & Cookies?','','','No', 'Yes'):
            import os
            ...
            xbmc.executebuiltin("XBMC.Notification(Clear Bedaub's Cache,Successful,5000,"")")
...
def DLLIBRTMP(mname,key,trigger):
    ...
    if re.search('(?i)linux',trigger):
        ...
        if (keyb.isConfirmed()):
            ...
            os.remove(lib)
    if re.search('APKINSTALLER',trigger):
        dialog.ok("Bedaub", "Thats It All Done", "[COLOR blue]Download location[/COLOR]",path) 
    else:
        dialog.ok("Bedaub", "Thats It All Done", "[COLOR blue]Now should be Updated[/COLOR]")
...
def MAINTENANCE(name):
    if name == 'MAINTENANCE':
        ...
        main.addSpecial('Clear Bedaub Cache & Cookies','MashCache',416,art+'/maintenance.png')
    else:
...
def FIXES():
    ...
    try:
        link=main.OPENURL('https://raw.github.com/bedaub/BedaubFixes/master/Fixes.xml')
    except:
...
def FIXDOWN(name,filename,location,path):
    ...
    url = 'https://raw.github.com/bedaub/BedaubFixes/master/FIXES/'+filename
    ...
    dialog.ok("Bedaub", "Thats It All Done", "[COLOR blue]Now "+name+" should be Fixed[/COLOR]")

def ADSettings(name,filename):
    main.GA("Fixes",name+"-Fix")
    url = 'https://raw.github.com/bedaub/BedaubFixes/master/AdvancedSettings/'+filename
    print "#############  Downloading from "+ url+"  #####################"
    ...
    dialog = xbmcgui.Dialog()
    name  = name.split('[COLOR red]')[0]
    dialog.ok("Bedaub", "Thats It All Done", "[COLOR blue]Now "+name+" should be Installed[/COLOR]")

def delAS():
    dialog = xbmcgui.Dialog()
    if dialog.yesno('Bedaub', 'Are you sure you want to remove advancedsettings.xml?','','','No', 'Yes'):
        path = xbmc.translatePath(os.path.join('special://home/','userdata'))
        file=os.path.join(path, 'advancedsettings.xml')
        os.remove(file)
        dialog.ok("Bedaub", "Thats It All Done", "[COLOR blue]Now AdvancedSettings.xml is removed[/COLOR]")
...
def showLiveAnnouncements():
    #Announcement Notifier from xml file
    try:
        link=main.OPENURL('https://raw.github.com/bedaub/BedaubNotifications/master/NotifierLive.xml', verbose=False)
    except: link='nill'
    link=link.replace('\r','').replace('\n','').replace('\t','').replace('&nbsp;','')
...
def AddToDixie(secName,name,murl,secIcon):
    ...
    if ret == -1:
        ...
        if not os.path.exists(SectionSource):
            ...
            open(SectionSource,'a').write("\n"+id+"="+path)
            xbmc.executebuiltin("XBMC.Notification(Bedaub, Successfully Added,2500,"+main.slogo+")")
        else:
            path = murl
            id = finalList[ret]
            open(SectionSource,'a').write("\n"+id+"="+path)
...
def downloadFileWithDialog(url,dest):
    try:
        ...
        dp.create("Bedaub","Downloading & Copying File",'')
        urllib.urlretrieve(url,dest,lambda nb, bs, fs, url=url: main._pbhook(nb,bs,fs,dp,time.time()))
    except Exception, e:
        dialog = xbmcgui.Dialog()
        main.ErrorReport(e)
        dialog.ok("Bedaub", "Report the error below at " + main.supportsite, str(e), "We will try our best to help you")
...
try: 
    if not os.path.exists(repopath):
        ...
        ## TODO: Fix
        ##########
        url = 'https://dl.dropboxusercontent.com/u/35068738/xxxx/repository.divingmule.addons.zip'
...
class HUBx( xbmcgui.WindowXMLDialog ):
    ...
    def _close_dialog( self ):
        path = xbmc.translatePath(os.path.join('special://home/addons/plugin.video.bedaub/resources/skins/DefaultSkin','media'))
...
def getFavorites(section_title = None):
    ...
    if len(fav_items) > 0:
        ...
        for fav_item in fav_items:
            ...
            xbmc.executebuiltin("XBMC.Notification([B][COLOR=FF67cc33]Bedaub[/COLOR][/B],[B]You Have No Saved Favourites[/B],5000,"")")
    return
...
def WHClear(url):
    dialog = xbmcgui.Dialog()
    ret = dialog.yesno('Bedaub Watch History', 'Are you sure you want to clear your','watch history, you can not restore','once you press yes','No', 'Yes')
    if ret:
        os.remove(url)
        xbmc.executebuiltin("XBMC.Container.Refresh")
...
def History():
    ...
    if selfAddon.getSetting("whistory") == "true":
        ...
        dialog = xbmcgui.Dialog()
        ok=dialog.ok('[B]Bedaub History[/B]', 'Watch history is disabled' ,'To enable go to addon settings','and enable Watch History')
        history_items = wh.get_my_watch_history()
        for item in history_items:
            item_title = item['title']
            item_url = item['url']
...
if mode==None or url==None or len(url)<1:
    ...
    from resources.libs import bedaub
    ...
    from resources.libs import bedaub
    print ""+url
    bedaub.Searchhistory()

elif mode==3:
    from resources.libs import bedaub
    print ""+url
    bedaub.VIDEOLINKS(name,url)

elif mode==5:
    from resources.libs import bedaub
    print ""+url
    bedaub.PLAY(name,url)

elif mode==171:
    from resources.libs import bedaub
    print ""+url
    bedaub.PLAYB(name,url)
elif mode==6:
    AtoZ()
...
#     name = re.sub('(?i)[^a-zA-Z0-9]',' ',name)
    ...
    from resources.libs import bedaub
    print ""+url
    bedaub.GroupedHosts(name,url,iconimage)
...
######39-42 available
    ...
    from resources.libs import bedaub
    print ""+url
    bedaub.MOVSHLINKS(name,url)

elif mode==146:
    from resources.libs import bedaub
    print ""+url
    bedaub.DIVXSLINKS(name,url)

elif mode==147:
    from resources.libs import bedaub
    print ""+url
    bedaub.SSIXLINKS(name,url)

elif mode==148:
    from resources.libs import bedaub
    print ""+url
    bedaub.GORLINKS(name,url)

elif mode==150:
    from resources.libs import bedaub
    print ""+url
    bedaub.MOVPLINKS(name,url)

elif mode==151:
    from resources.libs import bedaub
    print ""+url
    bedaub.DACLINKS(name,url)

elif mode==152:
    from resources.libs import bedaub
    print ""+url
    bedaub.VWEEDLINKS(name,url)

elif mode==153:
    from resources.libs import bedaub
    print ""+url
    bedaub.MOVDLINKS(name,url)

elif mode==154:
    from resources.libs import bedaub
    print ""+url
    bedaub.MOVRLINKS(name,url)

elif mode==155:
    from resources.libs import bedaub
    print ""+url
    bedaub.BUPLOADSLINKS(name,url)
...
#171 taken
    ...
    dialog = xbmcgui.Dialog()
    dialog.ok("Bedaub", "Sorry this video requires a SkySports Suscription.","Will add this feature in later Version.","Enjoy the rest of the videos ;).")
