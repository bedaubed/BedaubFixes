...
def showChangeLog(env):
    text = ''
    if env.lower() == 'dev':
        url = 'https://github.com/bedaub/Bedaub/commits/master'
        env = 'Dev'
        ...
        if selfAddon.getSetting("autosource") == "false":
            if selfAddon.getSetting("updateoption") == "gitupdate1":
                url = 'https://github.com/bedaub/gitupdate1/commits/master'
            elif selfAddon.getSetting("updateoption") == "gitupdate2":
                url = 'https://github.com/bedaub/gitupdate2/commits/master'
            else:
                url = 'https://github.com/bedaub/AutoUpdate/commits/master'
	else:
	    import time
	    url='https://bitbucket.org/!api/2.0/repositories/bedaub/bitautoupdate1/commits?'+ str(time.time())
	env = ''
